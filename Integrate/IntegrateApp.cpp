...

	{
		...
		while ( !exit_ ) {
			...
			if ( triggered_capture ) {
                ...
                //cout << "Be careful. I disable the trigger function from ONIGrabber here. Will it affect?" << endl;
				// SH: looks like there is no trigger function for ONIGrabber
				//( ( pcl::PCDGrabber<pcl::PointXYZRGB> * ) &capture_ )->trigger(); // Triggers new frame

                //( ( pcl::ONIGrabber * ) &capture_ )->
...
{
	{
        ...
        int oniFrameID = depth_wrapper->getDepthMetaData().FrameID();

        if (frame_id_ == 0 || traj_.data_[ frame_id_ - 1 ].frame_ < oniFrameID) {
            ...
            while (traj_.data_[ frame_id_ - 1 ].frame_ < oniFrameID);
        }

        // after increasing frame_id_, the frame number can be larger than ONI frame
        // then we wait for the correct ONI frame to come
        if (frame_id_ > 0 && traj_.data_[ frame_id_ - 1 ].frame_ > oniFrameID) {
            //cout << "Mismatch frame: " << traj_.data_[ frame_id_ - 1 ].frame_ << " vs. " << oniFrameID << endl;
            return;
        }
        ...
        cout << "Index " << frame_id_ << " for ONI frame " << oniFrameID << '\r' << std::flush;
