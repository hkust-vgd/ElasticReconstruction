...
COptApp::COptApp(void)
	...
	, dir_prefix_("./pcds/")
	, rgbd_filename_("init.log")
	, reg_filename_("reg_output.log")
{
}
...
{
	// from the initial blacklist, must discard fragments that do not have correspondences also
	// reg_traj is from reg_output.log
	std::vector<bool> has_corres(num_);		// num_ is original fragments, will be reassigned after determining blacklist
	for (int i = 0; i < has_corres.size(); ++i) has_corres[i] = false;
	...
	for (int i = 0; i < (int)reg_traj_.data_.size(); i++) {
		...
		if (reg_traj_.data_[i].frame_ != -1 && reg_traj_.data_[i].frame_ >= blacklist_pair_num_) {
			has_corres[reg_traj_.data_[i].id1_] = true;
			has_corres[reg_traj_.data_[i].id2_] = true;
		}
	}

	// finalize the blacklist fragments
	for (int i = 0; i < has_corres.size(); ++i) {
		if (!has_corres[i]) {
			if (blacklist_.find(i) == blacklist_.end()) {
				blacklist_.insert(i);
				std::cout << "Fragment " << i << " is added to blacklist due to lack of corresponences." << std::endl;
			}
		}
	}
	std::cout << "Final blacklist: ";
	for (int i = 0; i < has_corres.size(); ++i) {
		if (blacklist_.find(i) != blacklist_.end())
			std::cout << i << " ";
	}
	...
	// init mapping to discard blacklist fragments
...
void COptApp::InitIPose()
{
    ...
    // this must match with base pose in init.log
	Eigen::Matrix4d basepose = Eigen::Matrix4d::Identity();         // world to camera 0
	
	// SH: remove hard code. 
	// use pose 0 in init.log as the basepose
	/*
	basepose( 0, 3 ) = length_ / 2.0;
	basepose( 1, 3 ) = length_ / 2.0;
	basepose( 2, 3 ) = - 0.3;
	*/
	basepose = rgbd_traj_.data_[0].transformation_;

	Eigen::Matrix4d baseinverse = basepose.inverse();               // camera 0 to world

    // rgbd_traj from the beginning stores poses for each fragment. Each pose is relative to the pose of the first frame
    // in the fragment
	Eigen::Matrix4d leftbase = basepose * rgbd_traj_.data_[ 0 ].transformation_.inverse();      
	ipose_.resize( num_ );          // camera to world
	pose_.resize( num_ );           // world to camera
	pose_rot_t_.resize( num_ );

    cout << "RGBD traj size: " << rgbd_traj_.data_.size() << endl;
	for ( int i = 0; i < num_; i++ ) {
		int ii = relative2absolute_map_[ i ];
        cout << "ii * interval_:  " << ii * interval_ << endl;
...
void COptApp::OptimizeRigidGaussNewton()
{
	PCL_WARN( "Gauss Newton optimization.\n" );
    ...
    // roughly transform the fragments to world based on initial pose
	...
	for ( int itr = 0; itr < max_iteration_; itr++ ) {
        // we can save points in each iteration to verify that the alignment works
        if (itr == 0 || itr == max_iteration_ - 1) {
            cout << "Dumping sample points in iteration " << itr << "..." << endl;
            ostringstream iteration_filename;
            iteration_filename << "iteration_" << itr << ".pcd";
            SavePoints( iteration_filename.str() );
        }
...
//#pragma omp critical
	...
	*/
	...
	// SH: output all poses include the blacklist fragments
	output_traj_.data_.clear();
	for (int i = 0; i < absolute2relative_map_.size(); i++) {
		if (absolute2relative_map_[i] == -1) {
			output_traj_.data_.push_back(FramedTransformation(i, i, -1, Eigen::Matrix4d::Zero()));
		}
		else {
			int k = absolute2relative_map_[i];
			output_traj_.data_.push_back(FramedTransformation(i, i, i + 1, pose_[k]));
		}
	}
	output_traj_.SaveToFile(pose_filename_);
	...
	Pose2Ctr( pose_, ctr );
	SaveCtr( ctr, ctr_filename_ );          // dummy control points
	SavePoints( ctr, sample_filename_ );
}

double COptApp::ComputeScore() {
	double thisscore = 0.0;
...
void COptApp::OptimizeRigidLevenbergMarquardt()
{
	PCL_WARN( "Levenberg-Marquardt optimization.\n" );
	PCL_INFO( "Parameters: resolution %d, piece number %d, max iteration %d\n", resolution_, num_, max_iteration_ );
    ...
    // Levenberg-Marquart
    const double e1 = 1e-12;
    const double e2 = 1e-12;
    const double e3 = 1e-12;
    const double e4 = 0.0;
    double tau = 1e-3;
    double v = 2.0;
    Eigen::VectorXd x = Eigen::VectorXd::Zero(matrix_size_);
    double lambda = 0.0;

    SparseMatrix identity(matrix_size_, matrix_size_);
    identity.setZero();
    for (int i = 0; i < matrix_size_; ++i) 
        identity.coeffRef(i, i) = 1.0;

	for ( int itr = 0; itr < max_iteration_; itr++ ) {

        PCL_INFO("Iteration %03d\n", itr);
        // we can save points in each iteration to verify that the alignment works
        //if (itr == 0 || itr == max_iteration_ - 1) {
            cout << "Dumping sample points in iteration " << itr << "..." << endl;
            ostringstream iteration_filename;
        ...
        // in fact we are solving Jt J x = - Jb
        // notation same as g2o
        thisJb = -thisJb;
        
        Eigen::VectorXd g = thisJb;        // equivalent to gradient descent direction
        if (g.cwiseAbs().maxCoeff() < e1) {
            PCL_INFO("Gradient small enough. Done.");
            break;
        }

        if (itr == 0)  // compute init lambda
            lambda = tau * thisJJ.diagonal().cwiseAbs().maxCoeff();
        else {
            // keep previous lambda
        }
        PCL_INFO("Lambda: %f\n", lambda);
        
        // error vector size is equal to total number of correspondences

        Eigen::VectorXd result;
        double rho = 0.0;
        // solve the damped version of JJ x = Jb
        // and adjust lambda
        do {
            SparseMatrix thisJJdamped = thisJJ + lambda * identity;
            Eigen::CholmodSupernodalLLT< SparseMatrix, Eigen::Upper > solver;
		    solver.analyzePattern( thisJJdamped );
		    solver.factorize( thisJJdamped );

    		result = solver.solve( thisJb );
            
            // apply new pose and compute new score
            for ( int l = 0; l < num_; l++ ) {
			    Eigen::Affine3d aff_mat;
			    aff_mat.linear() = ( Eigen::Matrix3d ) Eigen::AngleAxisd( result( l * 6 + 2 ), Eigen::Vector3d::UnitZ() )
				    * Eigen::AngleAxisd( result( l * 6 + 1 ), Eigen::Vector3d::UnitY() )
            ...
            double new_score = ComputeScore();
            PCL_INFO( "New score is : %.2f\n", new_score );

            rho = (thisscore - new_score) / result.dot(lambda * result + g);
            PCL_INFO("Rho: %.2f\n", rho);
            if (rho > 0) {      
                // shrink lambda (less damping). Move closer to Gauss Newton.

                lambda = lambda * std::min(0.67, std::max(0.33, 1.0 - pow(2.0 * rho - 1.0, 3)));
                v = 2;
                PCL_INFO("Shrink lambda to %f\n", lambda);

                // go to next round with new pose and Jacobian

            } else {    

                lambda = lambda * v;
                v = 2 * v;
                PCL_INFO("Increase lambda to %f\n", lambda);

                // solve again with larger lambda. Move closer to gradient descent
                for ( int l = 0; l < num_; l++ ) {

                    // discard the delta pose applied above. 
                    // this could yield some inconsistency as JJ and Jb is not recomputed, but should work in practice.
			        Eigen::Affine3d aff_mat;
			        aff_mat.linear() = ( Eigen::Matrix3d ) Eigen::AngleAxisd( result( l * 6 + 2 ), Eigen::Vector3d::UnitZ() )
				        * Eigen::AngleAxisd( result( l * 6 + 1 ), Eigen::Vector3d::UnitY() )
				        * Eigen::AngleAxisd( result( l * 6 + 0 ), Eigen::Vector3d::UnitX() );
			        aff_mat.translation() = Eigen::Vector3d( result( l * 6 + 3 ), result( l * 6 + 4 ), result( l * 6 + 5 ) );
                    pose_[ l ] = aff_mat.matrix().inverse() * pose_[ l ];
                    pointclouds_[ l ].UpdatePose( aff_mat.matrix().inverse().cast< float >() );
		        }
            }
        } while (rho <= 0.0);
        ...
        //if (solver.info() == Eigen::NumericalIssue) {
            PCL_INFO("Numerical issue in linear solver. Should check the Jacobian.\n");
            PCL_INFO("No optimization done. Quitting.");
            ofstream f("jacobian.txt");
            for (int i = 0; i < thisJJ.rows(); ++i) {
                for (int j = 0; j < thisJJ.cols(); ++j) {
                    f << thisJJ.coeff(i, j) << " ";
                }
                f << '\n';
            }
            ...
            ofstream r("right.txt");
            for (int i = 0; i < thisJb.size(); ++i) {
                r << thisJb[i] << '\n';
            }
            r.close();
            break;
...
{
    OptimizeRigidGaussNewton();
    //OptimizeRigidLevenbergMarquardt();
}
...
void COptApp::SavePoints( std::string filename )
{
	if ( sample_num_ <= 0 )
		return;
	...
	for ( int l = 0; l < num_; l++ ) {
		for ( int i = 0; i < ( int )pointclouds_[ l ].points_.size(); i += sample_num_ ) {
            Eigen::Vector3d pos ( pointclouds_[ l ].points_[ i ].p_[ 0 ], pointclouds_[ l ].points_[ i ].p_[ 1 ], pointclouds_[ l ].points_[ i ].p_[ 2 ] );
	...
	PCL_INFO( "Save sample pcd into %s ... \n ", filename.c_str() );
	if (w.writeBinaryCompressed(filename, pcd) < 0) {
...
void COptApp::SavePoints( const Eigen::VectorXd & ctr, std::string filename )
{
    ...
    pcl::PCDWriter w;
	pcl::PointCloud< pcl::PointXYZRGBNormal > pcd;
	pcl::PointCloud< pcl::PointXYZRGBNormal > fragment_pcd;     // output each fragment separately
	pcl::PointXYZRGBNormal pt;
	for ( int l = 0; l < num_; l++ ) {
        fragment_pcd.clear();
		...
		for ( int i = 0; i < ( int )pointclouds_[ l ].points_.size(); i += sample_num_ ) {
            ...
            //fragment_pcd.push_back(pt);
		}

        /*
        ostringstream fragment_filename;
        fragment_filename << "sample" << l << ".pcd";
        PCL_INFO( "Save sample fragment pcd into %s_%d ... \n", fragment_filename.str().c_str(), l );
        w.writeBinaryCompressed( fragment_filename.str().c_str(), fragment_pcd );
        */
	}
